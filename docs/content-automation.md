...
결과 이미지는 범용 제너레이터로 만듭니다. draft를 만든 뒤(publish 전) 실행합니다.

```bash
pnpm gen:result-images -- --testId=<testId>
```

- 입력: `content/test-packs/drafts/<testId>.json`의 결과 code 목록
- 출력: `content/test-packs/packs/generated-v1/assets/<testId>/<code>.png`(960×640), `<code>-share.png`(1200×630)
- 결과 code의 해시로 팔레트 hue와 모티프(6종)를 정해, 결과마다 색·구도가 구분됩니다(색만 바꾼 복제가 아님).
- 한글 텍스트는 폰트 보장이 안 되므로 이미지에 넣지 않습니다.
- 의존성: Python 3 + Pillow. 없으면 `pip3 install Pillow`.

`scripts/gen-<topic>-images.py`(gift/shopping)는 초기 수작업 제너레이터로, 신규 테스트는 범용 제너레이터를 씁니다.
...
flowchart LR
  ...
  Draft --> Img["pnpm gen:result-images<br/>결과 이미지"]
  Img --> Check["pnpm check:content"]
...
- `pnpm gen:result-images` — 결과 이미지 범용 생성 (`scripts/gen-result-images.py`, Pillow 필요)
- 갱신된 `docs/prompts/new-test-pack-pr.md` step 0·step 3
